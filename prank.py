...
class prank(object):
    ...
    def learn_to_rank(self):
        ...
        new_label = [0 for x in range(self.rank_label + 1)]
        tao = []
        print self.source_data[0]
        ...
        for num in range(self.rank_iter):
            for i in tqdm(range(self.rank_num)):
                ...
                if self.source_data[i][0] != predict_rank:
                    for r in range(self.rank_label):
                        if self.source_data[i][0] - self.br[r] < 0:
                            new_label[r] = -1
                        else:
                            new_label[r] = 1

                tao = [new_label[r] if (
                    sumwx - self.br[r]) * new_label[r] <= 0 else 0.0 for x in range(self.rank_label)]
                tao_sum = sum(tao)
                new_weight = [self.weight[x] + tao_sum * self.source_data[i][x]
